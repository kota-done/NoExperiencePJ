...
- Java アプリケーション内部では、`DirectEmployee` 系と `ClientCompany` 系の機能を Controller / Service / Repository ごとに分けて管理する。
- ドメインモデルとして `DirectEmployee` と `ClientCompany` を扱い、従業員が所属する派遣先企業を関連付けて管理する。
...
flowchart LR
    subgraph LPC["Local PC"]
        ...
        S["Service Layer\nDirectEmployeeService / DirectEmployeeServiceImpl\nClientCompanyService / ClientCompanyServiceImpl"]
        M["Domain Model\nDirectEmployee\nClientCompany\n(client_company: charge/prefectureName含む)"]
